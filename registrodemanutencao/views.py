...
class aprovarListView(PermissionRequiredMixin, LoginRequiredMixin, ListView):
    model = registrodemanutencao
    template_name = "manutencao_aprovar.html"
    context_object_name = 'dasentradas'
    paginate_by = 12
...
#----------------------------------------------------------------------------
# View para listar todos os registros de manutenções realizadas para o Comercial
#-----------------------------------------------------------------------
class realizadasListView(PermissionRequiredMixin, LoginRequiredMixin, ListView):
    model = registrodemanutencao
    template_name = "manutencao_realizada.html"
    context_object_name = 'dasentradas'
    paginate_by = 12
    ...
    def get_queryset(self):
        ...
        if equipamentos_param:
            ...
            # Se não houver filtro de equipamento, aplica o filtro de status normal
            queryset = registrodemanutencao.objects.filter(
                status__in=['Aprovado pela Inteligência', 'Comercial']
            )
...
def aprovar_manut(request, id):
    from .tasks import enviar_relatorio_aprovacao_task
    registro = get_object_or_404(registrodemanutencao, id=id)
    registro.status = 'Aprovado Inteligência'
    registro.save()
    enviar_relatorio_aprovacao_task.delay(registro.id, ['comercial@grupogoldensat.com.br'])
...
def aprovar_manutencao(request, id):
    from .tasks import enviar_relatorio_aprovacao_task
    registro = get_object_or_404(registrodemanutencao, id=id)
    registro.status = 'Aprovado pela Inteligência'
    registro.save()
    enviar_relatorio_aprovacao_task.delay(
        registro.id,
        ['financeiro@grupogoldensat.com.br', 'comercial@grupogoldensat.com.br'],
    )
    messages.success(request, 'Manutencao aprovada! O relatorio sera enviado por email em instantes.')
...
def aprovar_manutencao_comercial(request, id):
    from .tasks import enviar_relatorio_aprovacao_task
    registro = get_object_or_404(registrodemanutencao, id=id)
    registro.status = 'Tratativa Finalizada'
    registro.save()
    enviar_relatorio_aprovacao_task.delay(
        registro.id,
        ['financeiro@grupogoldensat.com.br', 'comercial@grupogoldensat.com.br'],
    )
    messages.success(request, 'Tratativa finalizada! O relatorio sera enviado por email em instantes.')
